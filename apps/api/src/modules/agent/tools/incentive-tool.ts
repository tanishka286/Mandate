 ...
 *
 * Authoritative voucher/loyalty facts are loaded from persisted backend incentive
 * data. Caller-supplied incentive payloads are ignored to preserve the AI boundary.
 */
...
const defaultIncentivesService = new IncentivesService();
...
export function createEvaluateIncentivesTool(
  incentivesService: IncentivesService = defaultIncentivesService,
): AgentToolHandler {
  return async (args) => {
    try {
      const parsed = parseOrThrow(evaluateIncentivesArgsSchema, args);
      const grossMinor =
        parsed.voucher_basket?.gross_amount_minor ??
        parsed.loyalty_basket?.gross_amount_minor ??
        0;

      const bundle = await incentivesService.listAuthoritativeBundle(grossMinor);
      const vouchers = bundle.vouchers;
      const loyaltyRewards = bundle.loyalty_rewards;
...
export const evaluateIncentivesTool = createEvaluateIncentivesTool();
